...
impl CoinbaseParts {
    /// Build the coinbase for `work`, paying `payout_script`, leaving `extranonce_len`
    /// bytes in the scriptSig for the extranonce.
    pub fn build(
        work: &WorkTemplate,
        payout_script: &[u8],
        ...
        let script_len = work.coinbase_script_prefix.len() + extranonce_len;
